...
const Styles = () => (
  <style>{`
    ...
    *, *::before, *::after { box-sizing: border-box; margin: 0; padding: 0; }
    ...
    select { color: #D6E0F5; }
    option { background: #141D35; color: #D6E0F5; }
  `}</style>
);
...
export default function AuthPage({ onLogin }) {
  ...
  const handleLogin = async e => {
    ...
    try {
      ...
      // Step 2: Get user details including UUID
      const detailRes  = await fetch(`${BACKEND}/user-details/${encodeURIComponent(email)}`);
      const detailData = await detailRes.json();

      // Extract UUID - try multiple fields
      const userId = detailData.userId || detailData.id || null;

      if (!userId) {
        setError("Could not retrieve user ID. Please try again.");
        setLoading(false);
        return;
      }
      ...
      const userData = {
        ...
        username: data.username || detailData.username || name,
        userId:   userId,
      };
      ...
      setError("Cannot connect to server — is Flask running on port 5000?");
    }
    setLoading(false);
  };
      ...
      if (res.ok) {
        ...
        setTimeout(async () => {
          try {
            ...
            if (loginRes.ok) {
              const detailRes  = await fetch(`${BACKEND}/user-details/${encodeURIComponent(signEmail)}`);
              const detailData = await detailRes.json();
              const userId = detailData.userId || detailData.id;
              if (userId) {
                const userData = { email: signEmail, username: name, userId };
                localStorage.setItem("mi_user", JSON.stringify(userData));
                onLogin(userData);
              } else {
                setMode("login"); setEmail(signEmail); setError(""); setSuccess("");
              }
            }
          } catch { setMode("login"); setEmail(signEmail); }
        }, 1000);
      } else {
  ...
  return (
    <div style={{minHeight:"100vh",display:"flex",alignItems:"center",justifyContent:"center",fontFamily:"var(--sans)",position:"relative",paddingBottom:40}}>
      ...
      <div style={{position:"relative",zIndex:10,width:"100%",maxWidth:420,margin:"0 20px",animation:"fadeUp 0.5s ease both"}}>
        {/* Logo */}
        ...
        <div style={{background:"var(--bg2)",border:"0.5px solid var(--border2)",borderRadius:12,overflow:"hidden",boxShadow:"0 24px 60px rgba(0,0,0,0.4)"}}>
          ...
          <div style={{display:"grid",gridTemplateColumns:"1fr 1fr",borderBottom:"0.5px solid var(--border)"}}>
            {["login","signup"].map(m=>(
              <button key={m} onClick={()=>{setMode(m);setError("");setSuccess("");}}
                style={{padding:"14px",fontFamily:"var(--mono)",fontSize:10,letterSpacing:"0.12em",textTransform:"uppercase",cursor:"pointer",border:"none",background:mode===m?"var(--bg3)":"transparent",color:mode===m?"var(--accent)":"var(--text3)",borderBottom:`2px solid ${mode===m?"var(--accent)":"transparent"}`,transition:"all 0.2s"}}>
          ...
          <div style={{padding:"28px 28px 24px"}}>
            {error&&<div style={{background:"rgba(199,91,106,0.1)",border:"0.5px solid rgba(199,91,106,0.3)",borderRadius:6,padding:"10px 14px",marginBottom:16,fontSize:12,color:"var(--down)"}}>{error}</div>}
            {success&&<div style={{background:"rgba(76,175,138,0.1)",border:"0.5px solid rgba(76,175,138,0.3)",borderRadius:6,padding:"10px 14px",marginBottom:16,fontSize:12,color:"var(--up)"}}>{success}</div>}
            ...
            {mode==="login"&&(
              <form onSubmit={handleLogin} style={{display:"flex",flexDirection:"column",gap:16}}>
                ...
                <button type="submit" disabled={loading}
                  style={{marginTop:4,padding:"12px",background:loading?"var(--bg3)":"var(--accent)",color:loading?"var(--text3)":"#000",border:"none",borderRadius:6,fontSize:11,fontFamily:"var(--mono)",fontWeight:600,letterSpacing:"0.12em",cursor:loading?"default":"pointer",transition:"all 0.2s"}}>
                  {loading?"SIGNING IN...":"SIGN IN →"}
                </button>
                <div style={{textAlign:"center",fontSize:11,color:"var(--text3)",fontFamily:"var(--mono)"}}>
                  No account?{" "}
                  <button type="button" onClick={()=>{setMode("signup");setError("");}} style={{background:"none",border:"none",color:"var(--accent)",cursor:"pointer",fontSize:11,fontFamily:"var(--mono)"}}>Create one</button>
                </div>
              </form>
            )}
            ...
            {mode==="signup"&&(
              <form onSubmit={handleSignup} style={{display:"flex",flexDirection:"column",gap:14}}>
                ...
                <div>
                  <label style={labelStyle}>EMAIL</label>
                  <input value={signEmail} onChange={e=>setSignEmail(e.target.value)} type="email" placeholder="your@email.com" style={inputStyle}
                    onFocus={e=>e.target.style.borderColor="var(--accent)"} onBlur={e=>e.target.style.borderColor="var(--border2)"}/>
                </div>
                <div>
                  <label style={labelStyle}>PASSWORD</label>
                  <input value={signPass} onChange={e=>setSignPass(e.target.value)} type="password" placeholder="Min 6 characters" style={inputStyle}
                    onFocus={e=>e.target.style.borderColor="var(--accent)"} onBlur={e=>e.target.style.borderColor="var(--border2)"}/>
                </div>
                <div style={{padding:"12px 14px",background:"var(--bg3)",borderRadius:6,border:"0.5px solid var(--border)"}}>
                  ...
                  <div style={{display:"grid",gridTemplateColumns:"1fr 1fr 1fr",gap:8}}>
                    <div>
                      ...
                      <select value={goal} onChange={e=>setGoal(e.target.value)} style={{...inputStyle,padding:"7px 10px",fontSize:11,cursor:"pointer"}}>
                    ...
                    <div>
                      ...
                      <select value={risk} onChange={e=>setRisk(e.target.value)} style={{...inputStyle,padding:"7px 10px",fontSize:11,cursor:"pointer"}}>
                    ...
                    <div>
                      ...
                      <select value={horizon} onChange={e=>setHorizon(e.target.value)} style={{...inputStyle,padding:"7px 10px",fontSize:11,cursor:"pointer"}}>
                ...
                <button type="submit" disabled={loading}
                  style={{padding:"12px",background:loading?"var(--bg3)":"var(--accent)",color:loading?"var(--text3)":"#000",border:"none",borderRadius:6,fontSize:11,fontFamily:"var(--mono)",fontWeight:600,letterSpacing:"0.12em",cursor:loading?"default":"pointer",transition:"all 0.2s"}}>
                  {loading?"CREATING ACCOUNT...":"CREATE ACCOUNT →"}
                </button>
                <div style={{textAlign:"center",fontSize:11,color:"var(--text3)",fontFamily:"var(--mono)"}}>
                  Already have an account?{" "}
                  <button type="button" onClick={()=>{setMode("login");setError("");}} style={{background:"none",border:"none",color:"var(--accent)",cursor:"pointer",fontSize:11,fontFamily:"var(--mono)"}}>Sign in</button>
                </div>
              </form>
            )}
          </div>
